...
#define SAVE_FILE               "tetrisrecord.dat"

// 每个类型组件有4个方块，最长/宽也就是4
...
// 绘制或擦除单个类型组件，利用offset_*可以实现在游戏池或者预报区域进行绘制
...
// 当新的类型组件产生后，会要重绘右侧信息区域，包括预报和得分等
...
// 在指定方向轴以指定偏移量平移一个类型组件，如果没“碰壁”返回true，否则false
...
// 产生一个新的类型组件，从预报依次递补
// 注意forecasts[0]始终与游戏池当前类型组件一致，已经不是“预”报了，所以这个它其实是不会显示的
...
GameInfo *start_game(GameInfo *game) {
    ...
    while (true) {
        ...
        for (int frame = 0; frame <= FRAME_PER_ROW; frame++) {
            ...
            // 如果移动了，重绘当前活动类型组件，采取差量重绘法，先擦除旧的，在绘制新的，更加高效
            if (tetrimino_moved) {
